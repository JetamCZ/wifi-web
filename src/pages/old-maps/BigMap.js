...
class BigMap extends React.Component {
    ...
    getUpdateData(config) {
        ...
        AxiosInstance.get("/maps/" + this.id).then((res) => {
            ...
            if (this.state.print.state === printState.GETTING_DATA) {
                ...
                if (
                    d &&
                    d.lastSeens.filter((ls) => new Date(ls.date) > this.state.print.d).length >= res.data.beacons.length
                ) {
    ...
    render() {
        const active = this.state.data
            ? this.state.data.beacons.filter((b) => Format.diffInSeconds(new Date(b.lastSeenDate)) < 30)
            : 0
                    ...
                    <div className="print">
                        ...
                        {this.state.print.state === printState.SETTINGS_LAT_LON && (
                            <>
                                ...
                                <span className="btn btn-light" onClick={() => this.printSetLatLng()}>
                                    Nastavit polohu
                                </span>
                                <span
                ...
                <div className="info-box shadow">
                    ...
                    {active.length < 3 && this.state.data.devices.length === 0 && (
                        <div className="error">
                            ...
                            <div className="text">
                                Could not find devices, to show device we need to have 3 active beacons!
                            </div>
                        </div>
                    )}
