...
class BigMap extends React.Component {
    ...
    printSetLatLng() {
        ...
        this.setState({
            print: {...this.state.print, state: printState.GETTING_DATA, d: new Date()}
        })
    }
    ...
    updateDevices(devices) {
        ...
        devices = devices.filter(dev => (dev.pos && dev.pos.p === "*") || (dev.pos && dev.pos.p === this.state.pa) )
    ...
    getUpdateData(config) {
        ...
        AxiosInstance.get('/maps/'+this.id)
            .then(res => {
                const allowedMacs = [
                ...
                if(this.state.print.state === printState.GETTING_DATA) {
                    const d = res.data.devices.find(d => d.mac === this.state.print.dev.mac)

                    if(d) {
                        console.log(d.lastSeens, d.lastSeens.filter(ls => new Date(ls.date) > this.state.print.d))
                    }

                    if(d && d.lastSeens.filter(ls => new Date(ls.date) > this.state.print.d).length >= res.data.beacons.length) {
        ...
        return (
            <div className="com-bigmap">
                                ...
                                this.state.print.state === printState.SETTINGS_LAT_LON && (
                                   <>
                                       ...
                                       <p>
                                           ...
                                           x: {this.state.print.x} y: {this.state.print.y} p: {this.state.print.p}
                                       </p>
                                       <span className="btn btn-light" onClick={() => this.printSetLatLng()}>Nastavit polohu</span>
                                       <span className="btn btn-cancel" onClick={() => {
                                           this.setState({print: {
                                                   ...this.state.print,
                                                   state: printState.OFF
                                               }})
                ...
                <div className="info-box shadow">
                    ...
                    <div className="plans">
                    {
                        this.state.data && this.state.data.layers.map(p =>
                            <div className={this.state.pa === p.p ? "plan selected" : "plan"} onClick={() => this.showP(p.p)}>
                                {p.p}
                            </div>
                        )
                    }
