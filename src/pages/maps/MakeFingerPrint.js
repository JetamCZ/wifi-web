...
const MakeFingerPrint = (props) => {
    ...
    const updateFingerPrintData = () => {
        AxiosInstance.get('/localization/'+id+'/fingerprint/'+point.mac)
            .then((res) => {
    ...
    const send = () => {
        ...
        AxiosInstance.post('/localization/'+id+'/fingerprint', dataSend)
            .then((res) => {
                props.afterSave()
            })
    }
...

            {
                state === states.DONE && <>
                    {progressStates.SELECT_DEVICE === progress && <>
                        ...
                        <label className="form-control">
                            ...
                            <select ref={selectRef}>
                                {
                                    devices.map(device =>
                                        <option value={device.mac}>{device.name} ({device.mac})</option>
                                    )
                                }
                            </select>
                        </label>
                        ...
                        <div className="text-right">
                            <div className="btn success" onClick={selectDevice}>Potvrdit výběr zařízení(1/3)</div>
                        </div>
                    ...
                    {progressStates.SELECTING_PLACE === progress && <>
                        ...
                        <div className="act-beacon">
                            ...
                            <p></p>
                            {
                                data.plan.floors.map((lay) =>
                                    <div className="btn sm warning" onClick={() => changeFloor(lay.floor)}>Přesunout do: {lay.name}</div>
                                )
                            }
                        </div>
                        ...
                        <div className="text-right">
                            <div className="btn success" onClick={save}>Nastavit pozici zařízení (2/3)</div>
                        </div>
                    </>}
                    {progressStates.GETTING_DATA === progress && <>
                        <h1 className="name">Získavají se síli signálů od jednotlivých majáků</h1>
                        <p>Toto může chvíli trvat, je zapotřebí mít signál minimálně od 3 majáků. (pokud lokalizace nemá přiřazené 3 majáky tak je zapotřebí mít signál ke všem přiřazeným majkákům)</p>
                        ...
                        <table className="table">
                            ...
                            <tbody>
                                {
                                    fingerPrintData && data.beacons.map(beacon => {
                                        const b = fingerPrintData.find(meet => meet.deviceKey === beacon.deviceKey)
                                        ...
                                        return (
                                            <tr>
                                                ...
                                                <td>{rssi} db</td>
                                                <td>{b?.date && ('před '+formats.toHMSWords(new Date(b.date)))}</td>
                                            </tr>
                                        )
                                        }
                                    )
                        ...
                        <div className="text-right">
                            {
                                fingerPrintData.length >= 3 ||  fingerPrintData.length === data.beacons.length ? (
                                    <div className="btn success" onClick={send}>Odeslat data (3/3)</div>
                                ) : (
