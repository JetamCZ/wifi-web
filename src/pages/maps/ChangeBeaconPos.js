...
const ChangeBeaconsPos = (props) => {
    ...
    const save = () => {
        ...
        AxiosInstance.put('/localization/'+id+'/beacons', saveData)
            .then(() => {
                props.afterSave()
            })
    }
                ...
                state === states.DONE && <>
...

                    {
                        data.type === "NEAREST_FINGERPRINT" &&
                            <p>Úprava bodů na mapě nemá vliv na určování polohy zařízení. (fingerprinty nelze automaticky opravit)</p>
                    }
                        ...
                        <div className="act-beacon">
...

                            {
                                data.plan.floors.map((lay) =>
                                    <div className="btn sm warning" onClick={() => changeFloor(actBeacon._id, lay.floor)}>Přesunout do: {lay.name}</div>
                                )
                            }
                        </div>
                        )
                    ...
                    <div className="text-right">
                        <div className="btn success" onClick={save}>Uložit pozici majáků</div>
                    </div>
                </>
            }
