...
const People = () => {
  ...
  const update = () => {
    AxiosInstance.get("/organization/people").then((res) => {
      setPeople(res.data);
    });
  };
  ...
  return (
    <>
      <div className="table-wrapper">
        <table className="table">
          <thead>
            <tr>
              <th>
                <T id="page.pageOrganization.members.name" />
              </th>
              <th>
                <T id="page.pageOrganization.members.lastActivity" />
              </th>
              <th className="text-right">
                ...
                >
                  <T id="page.pageOrganization.members.invite" />
                </div>
              </th>
            </tr>
          </thead>
              ...
              people.map((person) => (
                <tr>
                  ...
                  <td>
                    <T id="before" />{" "}
                    {person.lastSeen
                      ? formats.toHMSWords(new Date(person.lastSeen))
                      : "?"}
                  </td>
                  <td className="text-right">
                    <div
                      className="btn sm"
                      onClick={() => showPerson(person._id)}
                    >
                      zobrazit člena
                    </div>
                  </td>
      ...
      <Modal ref={modal}>
        <h1>
          <T id="page.pageOrganization.members.modal.title" />
        </h1>
        <p>
          <T id="page.pageOrganization.members.modal.info" />
        </p>
        <GenerateInv />
      </Modal>
