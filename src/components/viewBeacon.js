...
const ViewBeacon = (props) => {
  ...
  const remove = () => {
    AxiosInstance.delete("/beacons/" + id).then(() => {
  ...
  const save = () => {
    AxiosInstance.put("/beacons/" + id, editData).then((res) => {
        ...
        <div className="modalView">
              ...
              <div className="value">
                {beaconData.lastSeenDate ? (
                  <>
                    ...
                    (<T id="before" />{" "}
                    {formats.toHMSWords(new Date(beaconData.lastSeenDate))})
                  </>
                ) : (
                  "?"
                )}
          ...
          <table className="table">
            <thead>
              <tr>
                <th>
                  <T id="viewBeacon.table.mac" />
                </th>
                <th>
                  <T id="viewBeacon.table.rssi" />
                </th>
                <th>
                  <T id="viewBeacon.table.lastActivity" />
                </th>
              </tr>
            </thead>
            <tbody>
              {beaconData.devices.map((device) => (
                <tr>
                  ...
                  <td>{device.rssi} db</td>
                  <td>
                    {formats.toDMYHMS(new Date(device.date))} (<T id="before" />{" "}
                    {formats.toHMSWords(new Date(device.date))})
                  </td>
                </tr>
              ))}
            </tbody>
